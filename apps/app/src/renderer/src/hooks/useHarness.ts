...
async function harnessPost<T = unknown>(
	path: string,
	body?: unknown,
): Promise<T> {
...
async function harnessGet<T = unknown>(
	path: string,
...
export function useHarness() {
	...
	const handleEvent = useCallback(
		(event: HarnessEvent & { threadId?: string }, isReplay = false) => {
			const s = stateRef.current;
			const eventThreadId = event.threadId;
			const isCurrentThread =
				!eventThreadId || eventThreadId === s.currentThreadId;
			...
			switch (event.type) {
				...
				case "agent_start":
					patch({
						...
						...(isReplay
							? {}
							: { toolStates: new Map(), subagentStates: new Map() }),
					});
					break;
				...
				case "user_message": {
					...
					const alreadyHas = s.messages.some(
						(m) =>
							m.role === "user" &&
							m.content.some((c) => c.type === "text" && c.text === content),
					);
					if (!alreadyHas) {
				...
				case "tool_end": {
					...
					const pendingApproval =
						s.pendingToolApproval?.toolCallId === event.toolCallId
							? null
				...
				case "subagent_text_delta": {
					...
					if (prev) {
						ss.set(event.toolCallId, {
							...prev,
							text: prev.text + event.textDelta,
						});
						patch({ subagentStates: ss });
					}
					break;
				...
				case "subagent_tool_start": {
					...
					if (prev) {
						...
						ss.set(event.toolCallId, {
							...prev,
							nestedTools: [...prev.nestedTools, tool],
						});
						patch({ subagentStates: ss });
					}
					break;
	...
	const resyncCurrentThread = useCallback(async () => {
		...
		try {
			...
			if (status.running) {
				...
				const data = await harnessGet<{ messages: HarnessMessage[] }>(
					"thread/messages",
					{ threadId },
				);
				patch({
	...
	const connect = useCallback(() => {
		...
		fetchEventSource(harnessUrl("events"), {
			...
			async onopen(response) {
				...
				throw new Error(
					`SSE open failed: ${response.status} ${response.statusText}`,
				);
			},
			...
			onmessage(msg) {
				...
				try {
					const event: HarnessEvent & { threadId?: string } = JSON.parse(
						msg.data,
					);
					handleEvent(event);
				} catch {
	...
	const sendMessage = useCallback(
		async (content: string, images?: { data: string; mediaType: string }[]) => {
			const threadId = stateRef.current.currentThreadId;
			if (!threadId) return;
	...
	const switchModel = useCallback(
		async (modelId: string, scope?: string, modeId?: string) => {
			const threadId = stateRef.current.currentThreadId;
			if (!threadId) return;
	...
	const respondToBackgroundQuestion = useCallback(
		async (threadId: string, questionId: string, answer: string) => {
			await harnessPost("answer", { threadId, questionId, answer });
			patch({
				backgroundNotifications:
					stateRef.current.backgroundNotifications.filter(
	...
	const respondToBackgroundToolApproval = useCallback(
		async (
			threadId: string,
	...
	const respondToBackgroundPlanApproval = useCallback(
		async (
			threadId: string,
			planId: string,
	...
	const createThread = useCallback(async (title?: string) => {
		const data = await harnessPost<{ threadId: string }>("thread/create", {
			title,
		});
		return data;
	}, []);
	...
	const switchThread = useCallback(
		async (threadId: string) => {
			...
			try {
				...
				patch({
					...
					pendingQuestion: status.pending.question
						? {
								questionId: status.pending.question.questionId,
								question: status.pending.question.question,
								options: status.pending.question.options,
							}
						: null,
					pendingToolApproval: status.pending.toolApproval
						? {
								toolCallId: status.pending.toolApproval.toolCallId,
								toolName: status.pending.toolApproval.toolName,
								args: status.pending.toolApproval.args,
							}
						: null,
					pendingPlanApproval: status.pending.planApproval
						? {
								planId: status.pending.planApproval.planId,
								title: status.pending.planApproval.title,
								plan: status.pending.planApproval.plan,
							}
						: null,
				});
				...
				if (status.runBuffer.length > 0) {
					const historyMessageIds = new Set(
						stateRef.current.messages.map((m) => m.id),
					);
					for (const event of status.runBuffer) {
						if (
							event.type === "message_end" &&
							"message" in event &&
							historyMessageIds.has(
			...
			if (stateRef.current.tasks.length === 0) {
				try {
					const threadState = await harnessGet<{ tasks?: TaskItem[] }>(
						"state",
						{ threadId },
					);
					...
					/* state endpoint may not be available */
				}
			}
	...
	const setPermissionCategory = useCallback(
		async (category: string, policy: PermissionPolicy) => {
			const threadId = stateRef.current.currentThreadId;
	...
	const setPermissionTool = useCallback(
		async (toolName: string, policy: PermissionPolicy) => {
			const threadId = stateRef.current.currentThreadId;
	...
	const init = useCallback(async () => {
		...
		const threadsData = await harnessGet<{ threads: HarnessThread[] }>(
			"thread/list",
		);
		...
		if (threads.length > 0) {
			...
			const created = await harnessPost<{ threadId: string }>("thread/create", {
				title: "New Thread",
			});
		...
		// Load modes
		try {
			const modesData = await harnessGet<{
	...
	const displayMessages: HarnessMessage[] = state.currentStreamingMessage
		? (() => {
				const idx = state.messages.findIndex(
					(m) => m.id === state.currentStreamingMessage!.id,
				);
				if (idx >= 0) {
					return state.messages.map((m, i) =>
						i === idx ? state.currentStreamingMessage! : m,
					);
				}
